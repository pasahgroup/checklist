						...
						<div class="container-fluid">
							<nav aria-label="breadcrumb">
								<ol class="breadcrumb bg-white mb-0 px-0 py-2">
									<li class="breadcrumb-item active" aria-current="page">Renting Items</li>
								</ol>
							</nav>
						</div>
					</div>
											...
											<div class="card card-custom gutter-b bg-transparent shadow-none border-0" >
												<div class="card-header align-items-center  border-bottom-dark px-0">
													<div class="card-title mb-0">
														<h3 class="card-label mb-0 font-weight-bold text-body">Items
														</h3>
													</div>
                                    ...
                                    <table id="orderTable" class="display" style="width:100%">
                                        ...
                                        <thead class="text-body">
                                            <tr>
                                                <th>ID</th>
                                                <th >Item</th>
                                                <th class="d-none d-lg-block">Unit</th>
                                                <th>Fee</th>
                                                <th class="no-sort text-right">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody class="kt-table-tbody text-dark">
                                            @foreach ($rent_items as $stock)
                                            <tr class="kt-table-row kt-table-row-level-0">
                                                <td >{{ $stock->id }}</td>
                                                <td>{{ $stock->item }}</td>
                                                <td class="d-none d-lg-block">{{ $stock->unit }}</td>
                                                <td>{{ number_format($stock->fee) }}</td>
                                                <td>
 <!-- start Modal -->
                ...
                <div class="row">
                    ...
                    <div class="col-12">
                        <div class="form-group">
                            <label class="text-dark" >Select Items </label>
                        ...
                        <div class="form-group">
                            <label class="text-dark" >Renting fee (Tsh) </label>
                            <input type="number" name="fee" class="form-control" required >
                            <small  class="form-text text-muted">please enter item renting feee</small>
                        </div>
                    </div>
                </div>
  ...
  {{-- end of modal --}}
	...
	<div  class="offcanvas offcanvas-right kt-color-panel p-5 kt_notes_panel">
		<div class="offcanvas-header d-flex align-items-center justify-content-between pb-3">
			<h4 class="font-size-h4 font-weight-bold m-0">Add Item
			</h4>
		...
		<form id="myform" action="{{ route('addrentItem') }}" method="POST">
            @csrf
			<div class="row">
