...
foreach ($arrayData as $key => $value) {
  ...
  //dd($count);
    ...
    $findValues =$value;
    if (in_array("Critical", $findValues))
      {
       $ans_label="Critical";
      }
      elseif (in_array("Non Critical", $findValues))
        {
       $ans_label="Non Critical";
        }
else{
  $ans_label="no_value";
       }
...
// if($count==1)
// {
//   if($value[$count-1]=='no_value')
//   {
//   $ans_label=$value[$count-1];
//   }
//   else {
//       $ans_label=$value[$count-1];
//   }
...
// if($count==2)
// {
//   if($value[$count-2]=='no_value')
//   {
//   $ans_label=$value[$count-2];
//   }
//   else {
//       $ans_label=$value[$count-1];
//   }
...
// if($count==3)
// {
//   if($value[$count-3]=='no_value')
//   {
//   $ans_label=$value[$count-3];
//   }
//   elseif($value[$count-2]=='no_value')
//   {
//   $ans_label=$value[$count-2];
//   }
//   else {
...
if($ans_label=="no_value")
{
//$answerTableUpdate1=DB::statement('update answers a,optional_answers o set a.answer=o.answer where a.opt_answer_id=o.id and a.indicator_id="'.$indicMeta[1] .'" and a.datex="'.$current_date .'" and a.section="'.$section.'" and a.property_id="'.$$property_id.'" and a.opt_answer_id="'.$property_id.'" and a.asset_id="'.$asset_id.'"');
$answerTableUpdate1=DB::statement('update answers a,optional_answers o set a.answer=o.answer where a.opt_answer_id=o.id and a.indicator_id="'.$indicMeta[1] .'" and a.datex="'.$current_date .'" and a.section="'.$section.'" and a.property_id="'.$property_id.'" and a.opt_answer_id="'.$value[0].'" and a.asset_id="'.$asset_id.'"');
...
$input =app_path().'/reports/pieChart.jrxml';
 //$ifvnput =app_path().'/reports/department.jrxml';
...
public function mount()
   {
    ...
    $this->metanames = metaname::join('qns_appliedtos','qns_appliedtos.metaname_id','metanames.id')
     ...
     // $assets = asset::where('assets.metaname_id',1)
...
$metanameCollects=collect($metanames);
      ...
      return view('livewire.checklist',compact('departGetName','metadatas','datatypes','assetID','metanames','metanameCollects','metaAll','pp','metas','qns','sections','userActitivities','acts','col','checkQnsProp','checkQns','assetPerc','answerPerc','answerCount','totalqns','qnsAppliedPerc','property_id'))
      ->layout('layouts.app');
